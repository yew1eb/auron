...
import org.apache.spark.sql.execution.auron.plan.{NativeShuffleExchangeBase, NativeSortMergeJoinBase}
...
class AuronKeyGroupedPartitioningSuite
  ...
  private def collectColumnarShuffleExchangeExec(
    ...
    collect(plan) {
      case s: NativeSortMergeJoinBase => s
      case s: SortMergeJoinExec => s
    }.flatMap(smj => collect(smj) { case s: NativeShuffleExchangeBase => s })
  }

  testAuron("partitioned join: only one side reports partitioning") {
  ...
  testAuron("partitioned join: exact distribution (same number of buckets) from both sides") {
    val customers_partitions = Array(bucket(4, "customer_id"))
  ...
  testAuron("partitioned join:  join with two partition keys and matching & sorted partitions") {
  ...
  testAuron("partitioned join: join with two partition keys and unsorted partitions") {
    val items_partitions = Array(bucket(8, "id"), days("arrive_time"))
    createTable(items, items_schema, items_partitions)
    sql(
      s"INSERT INTO testcat.ns.$items VALUES " +
  ...
  testAuron("partitioned join: join with two partition keys and different # of partition keys") {
    val items_partitions = Array(bucket(8, "id"), days("arrive_time"))
    createTable(items, items_schema, items_partitions)
  ...
  testAuron("data source partitioning + dynamic partition filtering") {
    withSQLConf(

...
package org.apache.spark.sql.execution
 ...
 import org.apache.spark.sql.{DataFrame, SparkQueryTestsBase}
...
class AuronReplaceHashWithSortAggSuite
  ...
  private def checkNumAggs(df: DataFrame, hashAggCount: Int, sortAggCount: Int): Unit = {
    ...
    assert(collectWithSubqueries(plan) {
      case s @ (_: NativeAggBase | _: ObjectHashAggregateExec) => s
    }.length == hashAggCount)
  ...
  testAuron("replace partial hash aggregate with sort aggregate") {
    withTempView("t") {
  ...
  testAuron("replace partial and final hash aggregate together with sort aggregate") {
    withTempView("t1", "t2") {
  ...
  testAuron("do not replace hash aggregate if child does not have sort order") {
    withTempView("t1", "t2") {
      spark.range(100).selectExpr("id as key").createOrReplaceTempView("t1")
      spark.range(50).selectExpr("id as key").createOrReplaceTempView("t2")
  ...
  testAuron("do not replace hash aggregate if there is no group-by column") {
    withTempView("t1") {
      spark.range(100).selectExpr("id as key").createOrReplaceTempView("t1")
      Seq("COUNT", "COLLECT_LIST").foreach {
        aggExpr =>

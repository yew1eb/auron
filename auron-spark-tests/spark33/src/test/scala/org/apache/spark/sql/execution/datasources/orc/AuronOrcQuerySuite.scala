...
class AuronOrcQuerySuite extends OrcQuerySuite with SparkQueryTestsBase {
  testAuron("Simple selection form ORC table") {
  ...
  testAuron("simple select queries") {
  ...
  testAuron("columns only referenced by pushed down filters should remain") {
  ...
  testAuron("SPARK-5309 strings stored using dictionary compression in orc") {
...
class AuronOrcV1QuerySuite extends AuronOrcQuerySuite {
  override def sparkConf: SparkConf =
    super.sparkConf
      .set(SQLConf.USE_V1_SOURCE_LIST, "orc")
}

class AuronOrcV2QuerySuite extends AuronOrcQuerySuite {
  override def sparkConf: SparkConf =
    super.sparkConf

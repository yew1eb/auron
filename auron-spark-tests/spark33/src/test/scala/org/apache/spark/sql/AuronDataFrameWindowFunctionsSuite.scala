...
class AuronDataFrameWindowFunctionsSuite
  ...
  testAuron("covar_samp, var_samp (variance), stddev_samp (stddev) functions in specific window") {
  ...
  testAuron("corr, covar_pop, stddev_pop functions in specific window") {
    withSQLConf(SQLConf.LEGACY_STATISTICAL_AGGREGATE.key -> "true") {
      val df = Seq(
        ("a", "p1", 10.0, 20.0),
        ("b", "p1", 20.0, 10.0),
...
//      case op@NativeShuffleExchangeExec =>

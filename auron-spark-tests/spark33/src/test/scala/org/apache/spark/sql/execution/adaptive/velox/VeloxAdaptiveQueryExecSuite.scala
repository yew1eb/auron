...
import org.apache.spark.sql.{Dataset, Row, SparkQueryTestsBase}
...
import org.apache.spark.sql.execution.auron.plan.{NativeBroadcastJoinBase, NativeShuffleExchangeBase, NativeShuffledHashJoinBase, NativeSortExec, NativeSortMergeJoinBase}

class VeloxAdaptiveQueryExecSuite extends AdaptiveQueryExecSuite with SparkQueryTestsBase {
  import testImplicits._

  override def sparkConf: SparkConf = {
    super.sparkConf
      //.set(AuronConfig.COLUMNAR_FORCE_SHUFFLED_HASH_JOIN_ENABLED.key, "false")
  ...
  private def findTopLevelSortMergeJoinTransform(
      plan: SparkPlan): Seq[NativeSortMergeJoinBase] = {
    collect(plan) { case j: NativeSortMergeJoinBase => j }
  }
  ...
  private def findTopLevelShuffledHashJoinTransform(
      plan: SparkPlan): Seq[NativeShuffledHashJoinBase] = {
    collect(plan) { case j: NativeShuffledHashJoinBase => j }
  }
  ...
  testAuron("Change merge join to broadcast join") {
    withSQLConf(
  ...
  testAuron("Change broadcast join to merge join") {
    withTable("t1", "t2") {
      withSQLConf(
  ...
  testAuron("Reuse the parallelism of coalesced shuffle in local shuffle read") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "300",
...
//  testAuron("Reuse the default parallelism in local shuffle read") {
//    withSQLConf(
//      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
//      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "300",
...
//  }
  ...
  testAuron("Empty stage coalesced to 1-partition RDD") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      ...
      withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1") {
        ...
        assert(find(plan)(_.isInstanceOf[NativeSortMergeJoinBase]).isDefined)
      }
      ...
      withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "1") {
        ...
        checkAnswer(testDf, Seq())
        val plan = testDf.queryExecution.executedPlan
        assert(find(plan)(_.isInstanceOf[NativeBroadcastJoinBase]).isDefined)
  ...
  testAuron("Scalar subquery in later stages") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "300") {
      val (plan, adaptivePlan) = runAdaptiveAndVerifyResult(
  ...
  testAuron("multiple joins with aggregate") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "300") {
  ...
  testAuron("multiple joins with aggregate 2") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
  ...
  testAuron("Exchange reuse with subqueries") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "300") {
      val (plan, adaptivePlan) = runAdaptiveAndVerifyResult(
      ...
      checkNumLocalShuffleReads(adaptivePlan)
      //      // Even with local shuffle read, the query stage reuse can also work.
      // Auron change the smj to bhj, stage is changed, so we cannot find the stage with old
  ...
  testAuron("Exchange reuse across subqueries") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "300",
  ...
  testAuron("Broadcast exchange reuse across subqueries") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      ...
      val ex = findReusedExchange(adaptivePlan)
      assert(ex.nonEmpty)
      assert(ex.head.child.isInstanceOf[ShuffleExchangeExec])
      val sub = findReusedSubquery(adaptivePlan)
      assert(sub.isEmpty)
    }
  }
  ...
  testAuron("Avoid plan change if cost is greater") {}

  testAuron("Change merge join to broadcast join without local shuffle read") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
  ...
  testAuron(
    ...
    withSQLConf(
      ...
      // because testData2 in Auron sizeInBytes(from ColumnarShuffleExchangeExec plan stats)
...
//        assert(bhj.head.joinBuildSide == BuildRight)
  ...
  testAuron("SPARK-30524: Do not optimize skew join if introduce additional shuffle") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
...
//          assert(innerSmj.size == 1 && innerSmj.head.isSkewJoin == optimizeSkewJoin)
  ...
  testAuron("SPARK-29544: adaptive skew join with different join types") {
...
//                case g: ShuffledHashJoinExecTransformerBase => assert(g.isSkewJoin)
  ...
  testAuron("SPARK-34682: AQEShuffleReadExec operating on canonicalized plan") {
  ...
  testAuron("metrics of the shuffle read") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.SHUFFLE_PARTITIONS.key -> "5") {
      val (_, adaptivePlan) = runAdaptiveAndVerifyResult("SELECT key FROM testData GROUP BY key")
      ...
      withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "300") {
        val (_, adaptivePlan) = runAdaptiveAndVerifyResult(
          "SELECT * FROM testData join testData2 ON key = a where value = '1'")
        val join = collect(adaptivePlan) { case j: NativeBroadcastJoinBase => j }.head
//        assert(join.joinBuildSide == BuildLeft)
  ...
  // because Auron use columnar format, which cannot execute to get rowIterator, then get the key
  // null status
  ignore(
    AURON_TEST + "SPARK-32573: Eliminate NAAJ when BuildSide is HashedRelationWithAllNullKeys") {}

  // EmptyRelation case
  ignore(
    AURON_TEST + "SPARK-35455: Unify empty relation optimization " +
      "between normal and AQE optimizer - single join") {}

  testAuron("SPARK-32753: Only copy tags to node with no tags") {
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true") {
      withTempView("v1") {
        ...
        assert(collect(adaptivePlan) { case s: NativeShuffleExchangeBase => s }.length == 1)
      }
    }
  }

  testAuron("Logging plan changes for AQE") {
  ...
  testAuron("SPARK-33551: Do not use AQE shuffle read for repartition") {
  ...
  testAuron("SPARK-34091: Batch shuffle fetch in AQE partition coalescing") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
  ...
  testAuron("SPARK-34899: Use origin plan if we can not coalesce shuffle partition") {
...
//      }.size == 1)
  ...
  testAuron("SPARK-35239: Coalesce shuffle partition should handle empty input RDD") {
    withTable("t") {
      withSQLConf(
        SQLConf.COALESCE_PARTITIONS_MIN_PARTITION_NUM.key -> "1",
        SQLConf.SHUFFLE_PARTITIONS.key -> "2",
  ...
  testAuron("SPARK-35264: Support AQE side broadcastJoin threshold") {
    withTempView("t1", "t2") {
  ...
  testAuron("SPARK-35264: Support AQE side shuffled hash join formula") {
    withTempView("t1", "t2") {
...
//                assert(shj.head.joinBuildSide == BuildRight)
  ...
  testAuron("SPARK-32932: Do not use local shuffle read at final stage on write command") {
    withSQLConf(
...
//  }
  ...
  testAuron("SPARK-35650: Coalesce number of partitions by AEQ") {
  ...
  testAuron("SPARK-35650: Use local shuffle read if can not coalesce number of partitions") {
  ...
  testAuron("SPARK-35725: Support optimize skewed partitions in RebalancePartitions") {
    withTempView("v") {
      withSQLConf(
        SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
        SQLConf.COALESCE_PARTITIONS_ENABLED.key -> "true",
        ...
        // Changed ADVISORY_PARTITION_SIZE_IN_BYTES from 150 to 120 because Auron has smaller
        // partition size.
  ...
  testAuron("SPARK-35888: join with a 0-partition table") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.COALESCE_PARTITIONS_MIN_PARTITION_NUM.key -> "1",
      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1",
  ...
  testAuron("SPARK-35968: AQE coalescing should not produce too small partitions by default") {
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true") {
      val (_, adaptive) =
  ...
  testAuron("SPARK-35794: Allow custom plugin for cost evaluator") {
  ...
  testAuron("SPARK-36020: Check logical link in remove redundant projects") {
    withTempView("t") {
      spark
        .range(10)
        .selectExpr(
  ...
  testAuron("SPARK-37742: AQE reads invalid InMemoryRelation stats and mistakenly plans BHJ") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",

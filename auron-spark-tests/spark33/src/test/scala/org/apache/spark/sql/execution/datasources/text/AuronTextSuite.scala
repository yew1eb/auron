...
import org.apache.spark.sql.{AnalysisException, DataFrame, SparkQueryTestsBase, QueryTest, Row, SaveMode}
...
abstract class AuronTextSuite
  ...
  testAuron("reading text file") {
    verifyFrame(spark.read.format("text").load(testFile))
  }

  testAuron("SQLContext.read.text() API") {
    verifyFrame(spark.read.text(testFile))
  }

  testAuron("SPARK-12562 verify write.text() can handle column name beyond `value`") {
  ...
  testAuron("error handling for invalid schema") {
    val tempFile = Utils.createTempDir()
    tempFile.delete()
  ...
  testAuron("reading partitioned data using read.textFile()") {
  ...
  testAuron("support for partitioned reading using read.text()") {
  ...
  testAuron("SPARK-13503 Support to specify the option for compression codec for TEXT") {
  ...
  testAuron("SPARK-13543 Write the output as uncompressed via option()") {
  ...
  testAuron("case insensitive option") {
    val extraOptions = Map[String, String](
      "mApReDuCe.output.fileoutputformat.compress" -> "true",
      "mApReDuCe.output.fileoutputformat.compress.type" -> CompressionType.BLOCK.toString,
      "mApReDuCe.map.output.compress" -> "true",
  ...
  testAuron("SPARK-14343: select partitioning column") {
    withTempPath {
      dir =>
  ...
  testAuron("SPARK-15654: should not split gz files") {
    withTempDir {
      dir =>
        val path = dir.getCanonicalPath
  ...
  def testLineSeparator(lineSep: String): Unit = {
    testAuron(s"SPARK-23577: Support line separator - lineSep: '$lineSep'") {
      // Read
      val values = Seq("a", "b", "\nc")
...
class AuronTextV1Suite extends AuronTextSuite with SparkQueryTestsBase {
  override def sparkConf: SparkConf =
    super.sparkConf
      .set(SQLConf.USE_V1_SOURCE_LIST, "text")
}

class AuronTextV2Suite extends AuronTextSuite with SparkQueryTestsBase {
  override def sparkConf: SparkConf =
    super.sparkConf

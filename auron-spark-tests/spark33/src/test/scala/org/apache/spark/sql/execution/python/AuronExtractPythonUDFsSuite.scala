...
class AuronExtractPythonUDFsSuite extends ExtractPythonUDFsSuite with SparkQueryTestsBase {
  ...
  testAuron("Chained Scalar Pandas UDFs should be combined to a single physical node") {
  ...
  testAuron("Mixed Batched Python UDFs and Pandas UDF should be separate physical node") {
    val df = Seq(("Hello", 4)).toDF("a", "b")
    val df2 = df
  ...
  testAuron("Dependent Batched Python UDFs and Scalar Pandas UDFs should not be combined") {
    val df = Seq(("Hello", 4)).toDF("a", "b")
    val df2 = df
      .withColumn("c1", batchedPythonUDF(col("a")))

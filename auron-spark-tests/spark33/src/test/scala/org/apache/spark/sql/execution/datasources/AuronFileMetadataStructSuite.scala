...
class AuronFileMetadataStructSuite extends FileMetadataStructSuite with SparkQueryTestsBase {
  ...
  private def metadataColumnsNativeTest(testName: String, fileSchema: StructType)(
    ...
    Seq("parquet").foreach {
      testFileFormat =>
        testAuron(s"metadata struct ($testFileFormat): " + testName) {
          withTempDir {
            dir =>
              ...
              val f1 = new File(dir, "data/f1 Auron").getCanonicalPath
              ...
              val realF0 = new File(dir, "data/f0")
                ...
                .head
              val realF1 = new File(dir, "data/f1 Auron")
                .listFiles()
                .filter(_.getName.endsWith(s".$testFileFormat"))
                .head

...
class AuronFileBasedDataSourceSuite extends FileBasedDataSourceSuite with SparkQueryTestsBase {
  ...
  testAuron("Option recursiveFileLookup: disable partition inferring") {
  ...
  testAuron("Spark native readers should respect spark.sql.caseSensitive - parquet") {
    withTempDir {
      dir =>
        val format = "parquet"
        ...
        withTable(tableName) {
          ...
          withSQLConf(SQLConf.CASE_SENSITIVE.key -> "false") {
            ...
            // TODO: Auron can catch exception in executor side, but cannot catch SparkException
            //  in Driver side
  ...
  testAuron("SPARK-22790,SPARK-27668: spark.sql.sources.compressionFactor takes effect") {
    Seq(1.0, 0.5).foreach {
      compressionFactor =>
        withSQLConf(
  ...
  testAuron("SPARK-25237 compute correct input metrics in FileScanRDD") {
  ...
  Seq("orc", "parquet").foreach {
    format =>
      testQuietly( s" Auron_TEST Enabling/disabling ignoreMissingFiles using $format") {

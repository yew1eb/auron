...
import org.apache.spark.sql.{QueryTest, SparkQueryTestsBase, SparkSession}
import org.apache.spark.sql.execution.adaptive.AdaptiveSparkPlanExec
import org.apache.spark.sql.functions.col
import org.apache.spark.sql.internal.SQLConf

class AuronCoalesceShufflePartitionsSuite
  ...
  override def withSparkSession(
    ...
    val sparkConf =
      new SparkConf(false)
        ...
        // Auron config
        .set("spark.plugins", "org.apache.Auron.AuronPlugin")
  ...
  Seq(Some(5), None).foreach {
    minNumPostShufflePartitions =>
      ...
      testAuron(s"determining the number of reducers: aggregate operator$testNameNote") {
        ...
        // Change the original value 2000 to 2500 for Auron. The test depends on the calculation
        // for bytesByPartitionId in MapOutputStatistics. Auron has a different statistic result.
        // See ShufflePartitionsUtil.coalescePartitions & AuronColumnarShuffleWriter's mapStatus.
        withSparkSession(test, 2500, minNumPostShufflePartitions)
      }

      testAuron(s"determining the number of reducers: join operator$testNameNote") {
        val test: SparkSession => Unit = {
          spark: SparkSession =>
            val df1 =
              spark
        ...
        // Change the original value 16384 to 20000 for Auron. The test depends on the calculation
        // for bytesByPartitionId in MapOutputStatistics. Auron has a different statistic result.
        // See ShufflePartitionsUtil.coalescePartitions & AuronColumnarShuffleWriter's mapStatus.
        withSparkSession(test, 20000, minNumPostShufflePartitions)
      }

      testAuron(s"determining the number of reducers: complex query 1$testNameNote") {
        val test: (SparkSession) => Unit = {
          spark: SparkSession =>
            val df1 =
              spark
      ...
      testAuron(s"determining the number of reducers: complex query 2$testNameNote") {
        val test: (SparkSession) => Unit = {
          spark: SparkSession =>
            val df1 =
              spark
        ...
        // Change the original value 12000 to 16000 for Auron. The test depends on the calculation
        // for bytesByPartitionId in MapOutputStatistics. Auron has a different statistic result.
        // See ShufflePartitionsUtil.coalescePartitions & AuronColumnarShuffleWriter's mapStatus.

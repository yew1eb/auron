...
abstract class AuronParquetFilterSuite extends ParquetFilterSuite with SparkQueryTestsBase {
  ...
  testAuron("filter pushdown - timestamp") {
  ...
  testAuron("SPARK-12218: 'Not' is included in Parquet filter pushdown") {
    import testImplicits._
  ...
  testAuron("SPARK-23852: Broken Parquet push-down for partially-written stats") {
    withSQLConf(SQLConf.PARQUET_FILTER_PUSHDOWN_ENABLED.key -> "true") {
  ...
  testAuron("SPARK-17091: Convert IN predicate to Parquet filter push-down") {
  ...
  testAuron("Support Parquet column index") {
    // block 1:
    //                      null count  min                                       max
    // page-0                         0  0                                         99
    // page-1                         0  100                                       199
...
@ExtendedSQLTest
class AuronParquetV1FilterSuite extends AuronParquetFilterSuite with SparkQueryTestsBase {
...
@ExtendedSQLTest
class AuronParquetV2FilterSuite extends AuronParquetFilterSuite with SparkQueryTestsBase {
  // TODO: enable Parquet V2 write path after file source V2 writers are workable.
  override def sparkConf: SparkConf =
    super.sparkConf
  ...
  testAuron("filter pushdown - date") {

...
class AuronParquetColumnIndexSuite extends ParquetColumnIndexSuite with SparkQueryTestsBase {
  ...
  testAuron("test reading unaligned pages - test all types") {
    val df = spark
      .range(0, 2000)
      .selectExpr(
        "id as _1",
  ...
  testAuron("test reading unaligned pages - test all types (dict encode)") {
    val df = spark
      .range(0, 2000)
      .selectExpr(
        "id as _1",

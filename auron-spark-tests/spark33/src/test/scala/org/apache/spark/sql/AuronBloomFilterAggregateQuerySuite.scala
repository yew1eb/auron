...
class AuronBloomFilterAggregateQuerySuite
  ...
  testAuron("Test bloom_filter_agg with big RUNTIME_BLOOM_FILTER_MAX_NUM_ITEMS") {
  ...
  testAuron("Test that might_contain on bloom_filter_agg with empty input") {
    checkAnswer(
  ...
  testAuron("Test bloom_filter_agg filter fallback") {
    val table = "bloom_filter_test"
    val numEstimatedItems = 5000000L
    val sqlString = s"""
    ...
    withTempView(table) {
      (Seq(Long.MinValue, 0, Long.MaxValue) ++ (1L to 200000L))
        ...
        .createOrReplaceTempView(table)
      withSQLConf(
        //AuronConfig.COLUMNAR_PROJECT_ENABLED.key -> "false"
      ) {
        val df = spark.sql(sqlString)
        df.collect
        assert(
          collectWithSubqueries(df.queryExecution.executedPlan) {
            case h if h.isInstanceOf[NativeAggBase] => h
          }.size == 2,
  ...
  testAuron("Test bloom_filter_agg agg fallback") {
    val table = "bloom_filter_test"
    val numEstimatedItems = 5000000L
    val sqlString = s"""
                       |SELECT col positive_membership_test
...
class AuronBloomFilterAggregateQuerySuiteCGOff extends AuronBloomFilterAggregateQuerySuite {

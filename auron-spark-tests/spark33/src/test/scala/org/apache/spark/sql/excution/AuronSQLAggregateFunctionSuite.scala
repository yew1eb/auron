...
class AuronSQLAggregateFunctionSuite extends SparkQueryTestsBase {
  ...
  testAuron("GLUTEN-4853: The result order is reversed for count and count distinct") {
    val query =
      """
    ...
    assert(getExecutedPlan(df).count(_.isInstanceOf[NativeAggBase]) == 4)
  }

  testAuron("Return NaN or null when dividing by zero") {
    val query =
      """

...
class AuronImplicitsTest extends SparkQueryTestsBase {
  //sys.props.put(AuronConfig.COLUMNAR_TABLE_CACHE_ENABLED.key, "true")
  ...
  testAuron("fallbackSummary with query") {
    withAQEEnabledAndDisabled {
      val df = spark.table("t1").filter(_.getLong(0) > 0)
     // assert(df.fallbackSummary().numAuronNodes == 1, df.fallbackSummary())
  ...
  testAuron("fallbackSummary with shuffle") {
    withAQEEnabledAndDisabled {
      val df = spark.sql("SELECT c2 FROM t1 group by c2").filter(_.getLong(0) > 0)
      //assert(df.fallbackSummary().numAuronNodes == 6, df.fallbackSummary())
      //assert(df.fallbackSummary().numFallbackNodes == 1, df.fallbackSummary())
      df.collect()
      //assert(df.fallbackSummary().numAuronNodes == 6, df.fallbackSummary())
      //assert(df.fallbackSummary().numFallbackNodes == 1, df.fallbackSummary())
  ...
  testAuron("fallbackSummary with set command") {
    withAQEEnabledAndDisabled {
      val df = spark.sql("set k=v")
      //assert(df.fallbackSummary().numAuronNodes == 0, df.fallbackSummary())
      //assert(df.fallbackSummary().numFallbackNodes == 0, df.fallbackSummary())
    }
  }

  testAuron("fallbackSummary with data write command") {
    withAQEEnabledAndDisabled {
      withTable("tmp") {
  ...
  testAuron("fallbackSummary with cache") {
    withAQEEnabledAndDisabled {
      val df = spark.table("t1").cache().filter(_.getLong(0) > 0)
      //assert(df.fallbackSummary().numAuronNodes == 2, df.fallbackSummary())
      //assert(df.fallbackSummary().numFallbackNodes == 1, df.fallbackSummary())
      df.collect()
      //assert(df.fallbackSummary().numAuronNodes == 2, df.fallbackSummary())
      //assert(df.fallbackSummary().numFallbackNodes == 1, df.fallbackSummary())
  ...
  testAuron("fallbackSummary with cached data and shuffle") {
    withAQEEnabledAndDisabled {
      val df = spark.sql("select * from t1").filter(_.getLong(0) > 0).cache.repartition()
      //assert(df.fallbackSummary().numAuronNodes == 7, df.fallbackSummary())
      //assert(df.fallbackSummary().numFallbackNodes == 1, df.fallbackSummary())
      df.collect()
      //assert(df.fallbackSummary().numAuronNodes == 7, df.fallbackSummary())
      //assert(df.fallbackSummary().numFallbackNodes == 1, df.fallbackSummary())

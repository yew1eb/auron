...
//      Row(new java.math.BigDecimal(2), new java.math.BigDecimal(6)) :: Nil)
  ...
  ignoreAuron("SPARK-32038: NormalizeFloatingNumbers should work on distinct aggregate") {
    withTempView("view") {
      Seq(
        ("mithunr", Float.NaN),
        ("mithunr", Float.NaN),
  ...
  testAuron("aggregation with filter") {
    Seq(
  ...
  testAuron("extend with cast expression") {
    checkAnswer(
      decimalData.agg(
        sum($"a".cast("double")),
        avg($"b".cast("double")),
  ...
  testAuron("use Auron hash agg to replace vanilla spark sort agg") {
      Seq("A", "B", "C", "D").toDF("col1").createOrReplaceTempView("t1")
      val df = spark.sql("select max(col1) from t1")
      checkAnswer(df, Row("D") :: Nil)
      // Sort agg is expected to be replaced by Auron's hash agg.
      assert(
        find(df.queryExecution.executedPlan)(
          _.isInstanceOf[NativeAggBase]).isDefined)
  }

  testAuron("mixed supported and unsupported aggregate functions") {
      ...
      withSQLConf(
        ...
        if (wholeStage) {
          assert(find(hashAggPlan) {
            ...
            // If offloaded, spark whole stage codegen takes no effect and a Auron hash agg is
            // expected to be used.

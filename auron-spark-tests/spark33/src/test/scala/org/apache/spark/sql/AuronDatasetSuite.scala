...
class AuronDatasetSuite extends DatasetSuite with SparkQueryTestsBase {
  import testImplicits._

  testAuron("dropDuplicates: columns with same column name") {
    ...
    // Using the checkDatasetUnorderly method to sort the result in Auron.
    checkDatasetUnorderly(joined.dropDuplicates(), (1, 2), (1, 1), (2, 1), (2, 2))
  }

  testAuron("groupBy.as") {
    ...
    val exchanges = collect(df3.queryExecution.executedPlan) {
      case h: NativeShuffleExchangeExec => h
    }
    // Assert the number of ColumnarShuffleExchangeExec
    // instead of ShuffleExchangeExec in Auron.

...
class AuronBatchEvalPythonExecSuite extends BatchEvalPythonExecSuite with SparkQueryTestsBase {

  import testImplicits._

  testAuron("Python UDF: push down deterministic FilterExecTransformer predicates") {
    val df = Seq(("Hello", 4))
      .toDF("a", "b")
...
//    assert(qualifiedPlanNodes.size == 2)
  ...
  testAuron("Nested Python UDF: push down deterministic FilterExecTransformer predicates") {
    val df = Seq(("Hello", 4))
      .toDF("a", "b")
...
//    assert(qualifiedPlanNodes.size == 2)
  ...
  testAuron("Python UDF: no push down on non-deterministic") {
    val df = Seq(("Hello", 4))
      .toDF("a", "b")

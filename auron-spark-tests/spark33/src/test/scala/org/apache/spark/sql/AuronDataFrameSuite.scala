...
import org.apache.spark.sql.execution.auron.plan.{NativeProjectBase, NativeShuffleExchangeBase}
...
class AuronDataFrameSuite extends DataFrameSuite with SparkQueryTestsBase {
  ...
  testAuron("reuse exchange") {
    withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "2") {
      ...
      assert(collect(join.queryExecution.executedPlan) {
        ...
        case e: NativeShuffleExchangeBase => true
      }.size === 1)
      assert(collect(join.queryExecution.executedPlan) {
  ...
  testAuron("Allow leading/trailing whitespace in string before casting") {
    def checkResult(df: DataFrame, expectedResult: Seq[Row]): Unit = {
      checkAnswer(df, expectedResult)
      assert(find(df.queryExecution.executedPlan)(_.isInstanceOf[NativeProjectBase]).isDefined)
    }

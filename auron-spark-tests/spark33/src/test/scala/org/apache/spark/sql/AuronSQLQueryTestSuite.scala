...
import org.apache.auron.utils.AuronSQLQueryTestSettings

class AuronSQLQueryTestSuite
    extends QueryTest
  ...
  protected val overwriteGoldenFilePath =
    new File(overwriteResourcePath, "results").getAbsolutePath
  ...
  def testCommandAvailable(command: String): Boolean = {
    val attempt = if (Utils.isWindows) {
      Try(
        Process(Seq("cmd.exe", "/C", s"where $command")).run(ProcessLogger(_ => ())).exitValue())
    } else {
      Try(
        Process(Seq("sh", "-c", s"command -v $command")).run(ProcessLogger(_ => ())).exitValue())
    }
  ...
  private val supportedList: Set[String] =
    AuronSQLQueryTestSettings.getSupportedSQLQueryTests ++
      AuronSQLQueryTestSettings.getOverwriteSQLQueryTests

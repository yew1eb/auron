...
import org.apache.spark.sql.{SparkQueryTestsBase, Row}
...
class AuronOrcSourceSuite extends OrcSourceSuite with SparkQueryTestsBase {
  ...
  testAuron("SPARK-31238: compatibility with Spark 2.4 in reading dates") {
    Seq(false).foreach {
      vectorized =>
  ...
  testAuron("SPARK-31238, SPARK-31423: rebasing dates in write") {
    withTempPath {
      dir =>
  ...
  testAuron("SPARK-31284: compatibility with Spark 2.4 in reading timestamps") {
    Seq(false).foreach {
      vectorized =>
        withSQLConf(SQLConf.ORC_VECTORIZED_READER_ENABLED.key -> vectorized.toString) {
          checkAnswer(
  ...
  testAuron("SPARK-31284, SPARK-31423: rebasing timestamps in write") {
    withTempPath {
      dir =>
        val path = dir.getAbsolutePath
  ...
  testAuron("SPARK-34862: Support ORC vectorized reader for nested column") {
    withTempPath {
      dir =>
        ...
        // Rewrite because Auron does not support Spark's vectorized reading.

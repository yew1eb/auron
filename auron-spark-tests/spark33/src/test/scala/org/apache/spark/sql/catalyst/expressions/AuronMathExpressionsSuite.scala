...
import org.apache.spark.sql.AuronQueryTestUtil.isNaNOrInf
...
class AuronMathExpressionsSuite extends MathExpressionsSuite with SparkQueryTestsBase {
...
//    if (BackendTestUtils.isVeloxBackendLoaded()) {
  ...
  testAuron("round/bround/floor/ceil") {
    val scales = -6 to 6
    ...
    scales.zipWithIndex.foreach {
      case (scale, i) =>
        ...
        checkEvaluation(
          ...
          if (true) floatResults(i) else bRoundFloatResults(i),
          EmptyRow
        )
        checkEvaluation(
    ...
    // Enable the test after fixing https://github.com/apache/incubator-Auron/issues/6827

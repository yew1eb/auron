 ...
 */
class AuronAggregateSuite extends AuronQueryTest with BaseAuronSQLSuite {
...
//        CometConf.COMET_SHUFFLE_MODE.key -> "jvm"
  ...
  // FIXME "AVG decimal supports emit.first"
  //  == Results ==
  //!== Correct Answer - 20 ==                    == Spark Answer - 20 ==
  // struct<_g1:tinyint,avg(_8):decimal(22,14)>   struct<_g1:tinyint,avg(_8):decimal(22,14)>
  // [null,null]                                  [null,null]
  // [-9,5.0672445E-7]                            [-9,5.0672445E-7]
  //![-8,5.0498788E-7]                            [-8,5.0498787E-7]
    ...
    withSQLConf(
      ...
      Seq(true, false).foreach { dictionaryEnabled =>
        withTempDir { dir =>
          ...
          withParquetTable(path.toUri.toString, "tbl") {
            logInfo(
              s"AVG decimal supports emit.first, tbl: ${spark.sql("select * from tbl limit 1").schema.toString}")
            checkSparkAnswerAndOperator(
  ...
  // FIXME "fix: Decimal Average should not enable native final aggregation"
  /**
   * ==Results==
   * !== Correct Answer - 20 == == Spark Answer - 20 == struct<_g1:tinyint,avg(_8):decimal(22,14)>
   * struct<_g1:tinyint,avg(_8):decimal(22,14)> ![-1,5.110741E-8] [-1,5.110740E-8]
   */
  ...
  // FIXME  "group-by on variable length types"
  ...
  // FIXME  "SUM, MIN, MAX, AVG for NaN, -0.0 and 0.0"
  //  == Results ==
  //!== Correct Answer - 1 ==                                      == Spark Answer - 1 ==
  // struct<min(_1):float,max(_1):float,min(_2):int,max(_2):int>   struct<min(_1):float,max(_1):float,min(_2):int,max(_2):int>
  //![0.0,NaN,1,4]                                                 [-0.0,-0.0,1,4]
...
// FIXME "SUM/MIN/MAX/AVG on decimal"
// poll record batch error: External error: Java exception thrown at native-engine/auron/src/rt.rs:239:
  ...
  // FIXME  "SUM decimal with DF"
  // Falling back expression: scala.NotImplementedError:
  // unsupported expression: (class org.apache.spark.sql.catalyst.expressions.UnscaledValue) UnscaledValue(_7#16673)
        ...
        withSQLConf(
          ...
          withTempDir { dir =>
            ...
            withParquetTable(path.toUri.toString, "tbl") {
              ...
              //StructField(_7,DecimalType(5,2),true)
              checkSparkAnswerAndNumOfAggregates(
  ...
  // FIXME
  // TODO  "multiple group-by columns + single aggregate column, with nulls"
  ...
  // FIXME (class org.apache.spark.sql.catalyst.expressions.objects.StaticInvoke)
  // TODO
  // 20:30:19.623 WARN org.apache.spark.sql.auron.NativeConverters: Falling back expression:
  // scala.NotImplementedError: unsupported expression:
  // (class org.apache.spark.sql.catalyst.expressions.objects.StaticInvoke)
  // staticinvoke(class org.apache.spark.sql.catalyst.util.
  // CharVarcharCodegenUtils, StringType, varcharTypeWriteSideCheck, col1#27018, 3, true, false, true)
  ...
  // very slow
  ignore("multiple group-by columns + multiple aggregate column, with nulls") {
    val numValues = 10000
...
// FIXME
  ...
  // FIXME
  // TODO 结果错误
  // 20:37:57.989 WARN org.apache.spark.sql.auron.NativeConverters: Falling back expression:
  // scala.NotImplementedError: unsupported expression:
  // (class org.apache.spark.sql.catalyst.expressions.CheckOverflowInTableInsert) cast(col1#196402 as decimal(38,37))

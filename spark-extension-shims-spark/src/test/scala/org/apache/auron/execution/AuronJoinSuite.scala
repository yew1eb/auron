...
class AuronJoinSuite extends AuronQueryTest with BaseAuronSQLSuite {
  ...
  test("SortMergeJoin with unsupported key type should fall back to Spark") {
    withSQLConf(
      ...
      withTable("t1", "t2") {
        ...
        println(cometPlan.canonicalized)
      }
    }
  }
  ...
  // FIXME  Broadcast HashJoin with join filter
  // https://github.com/apache/datafusion-comet/pull/211
  ...
  // https://github.com/apache/datafusion-comet/pull/735
  ...
  // FIXME
  // TODO
  // https://github.com/apache/datafusion-comet/pull/194/files
  // feat: Support HashJoin operator
  ...
  // FIXME "SortMergeJoin with join filter"
  // feat: Support sort merge join with a join condition
  // https://github.com/apache/datafusion-comet/pull/553

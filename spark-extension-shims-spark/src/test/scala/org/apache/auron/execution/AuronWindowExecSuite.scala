...
import org.apache.spark.sql.execution.auron.plan.{NativeShuffleExchangeExec, NativeWindowExec}
...
class AuronWindowExecSuite  extends AuronQueryTest with BaseAuronSQLSuite {
...
//        CometConf.COMET_EXEC_SHUFFLE_ENABLED.key -> "true",
    ...
    test("window query with rangeBetween") {
      ...
      val dfx = checkSparkAnswer(df2)
      val cometPlan = dfx.queryExecution.executedPlan
      val cometWindowExecs = collect(cometPlan) { case w: NativeWindowExec =>
        w
      }
          ...
          withTempPath { dir =>
            ...
            withParquetTable(path.toUri.toString, "tbl") {
              Seq(128, numValues + 100).foreach { batchSize =>
                withSQLConf("spark.auron.batchSize" -> batchSize.toString) {
                  (1 to 11).foreach { col =>

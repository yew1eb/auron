...
class AuronNativeShuffleSuite extends AuronQueryTest with BaseAuronSQLSuite {
  ...
  test("fix: Too many task completion listener of ArrowReaderIterator causes OOM") {
    withSQLConf("spark.auron.batchSize" -> "1") {
  ...
  // FIXME  "native shuffle: different data type"
  // schema_adapter_cast_column unsupported type: UInt64 => Decimal128(20, 0)
  ...
  test("fix: Dictionary arrays imported from native should not be overridden") {
    Seq(10, 201).foreach { numPartitions =>
      withSQLConf("spark.auron.batchSize" -> "10") {
  ...
  // FIXME  "fix: Comet native shuffle deletes shuffle files after query"

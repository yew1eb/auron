...
//  }
  ...
  test("TakeOrderedAndProject") {
    withTempView("t1") {
      ...
      withSQLConf("spark.sql.planChangeLog.level" -> "INFO") {
        ...
        var df = checkSparkAnswerAndOperator("SELECT id + 42 FROM t1 order by id limit 4")
  ...
  test("ShuffleQueryStageExec could be direct child node of NativeBroadcastExchangeExec") {
    withSQLConf(
  ...
  // FIXME exists-orderby-limit offset
  // TODO !== Correct Answer - 0 ==   == Spark Answer - 1 ==
  //!struct<>                   struct<key:int,value:string>
  //!                           [238,val_238]
  ...
  test("Sort on array of boolean") {
    withSQLConf(
      ...
      df.collect()
      println(df.queryExecution.executedPlan)
      val sort = stripAQEPlan(df.queryExecution.executedPlan).collect { case s: NativeSortExec =>
        s
      }.headOption
      assert(sort.isDefined)
    }
  }
  // FIXME LocalTableScanExec: Data type conversion not implemented TimestampNTZType
  // TODO
  ...
  // FIXME   Data type conversion not implemented TimestampNTZType
  // TODO
...
//        CometConf.COMET_EXEC_SHUFFLE_ENABLED.key -> "true",
  ...
  test("fix CometNativeExec.doCanonicalize for ReusedExchangeExec") {
    withSQLConf(
      ...
      withTable("td") {
        ...
        assert(reusedExchanges.head.child.isInstanceOf[NativeBroadcastExchangeExec])
      }
    }
  }

  test("ReusedExchangeExec should work on NativeBroadcastExchangeExec") {
    withSQLConf(
      //CometConf.COMET_EXEC_BROADCAST_FORCE_ENABLED.key -> "true",
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "false",
      ...
      Seq("native", "jvm").foreach { columnarShuffleMode =>
        withSQLConf(
          ...
          val shuffle = find(df.queryExecution.executedPlan) {
            case _: NativeShuffleExchangeExec if columnarShuffleMode.equalsIgnoreCase("jvm") =>
              true
  ...
  test("Fix corrupted AggregateMode when transforming plan parameters") {
    withParquetTable((0 until 5).map(i => (i, i + 1)), "table") {
      ...
      assert(agg.execMode.isInstanceOf[AggregateMode])
      val newAgg = agg.asInstanceOf[NativeAggExec]
    }
  }
  // FIXME
  // TODO
  test("NativeBroadcastExchangeExec") {
    withSQLConf(
      //CometConf.COMET_EXEC_BROADCAST_FORCE_ENABLED.key -> "true"
    ) {
      withParquetTable((0 until 5).map(i => (i, i + 1)), "tbl_a") {
        withParquetTable((0 until 5).map(i => (i, i + 1)), "tbl_b") {
          ...
          val nativeBroadcast = find(df.queryExecution.executedPlan) {
            case _: NativeBroadcastExchangeExec => true
            case _ => false
          }.get.asInstanceOf[NativeBroadcastExchangeExec]
  ...
  // FIXME
  // TODO
  test("NativeBroadcastExchangeExec: empty broadcast") {
    withSQLConf(
      //CometConf.COMET_EXEC_BROADCAST_FORCE_ENABLED.key -> "true"
    ) {
      withParquetTable((0 until 5).map(i => (i, i + 1)), "tbl_a") {
    ...
    withSQLConf(
      ...
      withTempDir { dir =>
        ...
        withParquetTable(path.toString, "tbl") {
          ...
          find(df.queryExecution.executedPlan)(s => s.isInstanceOf[NativeParquetScanExec])
            .foreach(scan => {
      ...
      withParquetTable((0 until 100).map(i => (i, i + 1)), "tbl_a") {
        withParquetTable((0 until 100).map(i => (i, i + 2)), "tbl_b") {
          withParquetTable((0 until 100).map(i => (i, i + 3)), "tbl_c") {
            ...
            // during AQE. This will make NativeBroadcastExchangeExec being converted to rows to be used by
            ...
            // There should be a NativeBroadcastExchangeExec under CometColumnarToRowExec
            ...
            assert(
              broadcastQueryStage.get
                ...
                .broadcast
                .isInstanceOf[NativeBroadcastExchangeExec])
          }
        }
      }
    }
  ...
  // FIXME
  // TODO TakeOrderedAndProjectExec with positive offset shoulf fallback
  ...
  test("TakeOrderedAndProjectExec only with offset") {
    withParquetTable((0 until 50).map(i => (i, i + 42)), "tbl") {
      val dfWithOffsetOnly = sql("SELECT _1, _2 FROM tbl order by _1 OFFSET 12")
      checkSparkAnswerAndOperator(dfWithOffsetOnly)
  ...
  // FIXME
  // TODO CollectLimit with offset should fallback
  ...
  // FIXME
  // TODO CollectLimit with offset shoulf fallback
  ...
  // FIXME
  // TODO  "native execution after coalesce"
  ...
  // struct<l:int,b:bigint>     struct<l:int,b:bigint>
  // [3,4]                      [3,4]
  // [0,1]                      [0,1]
  //![2,3]                      [4,5]
  //![4,5]                      [1,2]
  ...
  // FIXME
  // TODO feat: support TakeOrderedAndProjectExec with offset
  //  !== Correct Answer - 1 ==         == Spark Answer - 1 ==
  // struct<a:bigint,sum(b):bigint>   struct<a:bigint,sum(b):bigint>
  //![null,18]                        [null,20]
          ...
          "struct(id)").foreach { valueType =>
          {
            withSQLConf(
              ...
              withTempPath { dir =>
                ...
                checkSparkAnswerAndOperator(df.select("*").groupBy("key", "value").count())
                // includeClasses = Seq(classOf[CometSparkToColumnarExec]))
  ...
  test("read CSV file") {
    Seq("", "csv").foreach { v1List =>
      withSQLConf(
        ...
        spark.read
          .csv("spark-extension-shims-spark/src/test/resources/test-data/csv-test-1.csv")
  ...
  test("read JSON file") {
    Seq("", "json").foreach { v1List =>
      withSQLConf(
        ...
        spark.read
          .json("spark-extension-shims-spark/src/test/resources/test-data/json-test-1.ndjson")
          .createOrReplaceTempView("tbl")
  ...
  test("SparkToColumnar override node name for columnar input") {
    withSQLConf(
      ...
      withTempDir { dir =>
        ...
        val planAfter = df.queryExecution.executedPlan
        val nodeNames = planAfter.collect { case c: NativeProjectBase =>
          c.nodeName
        }
        println(planAfter)
        assert(nodeNames.length == 1)

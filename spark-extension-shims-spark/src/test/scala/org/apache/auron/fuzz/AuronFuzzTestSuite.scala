...
// WARN AuronConverters: Falling back exec: FileSourceScanExec:
  ...
  // FIXME Found non-native operator: CollectLimit
  // TODO
  ...
  // FIXME select column with default value
  // TODO
  ...
  // FIXME NotImplementedError: Data type conversion not implemented TimestampNTZType
  ...
  // TODO
  //25/12/02 08:08:44 WARN AuronConverters: Falling back exec:
  // FileSourceScanExec: Data type conversion not implemented TimestampNTZType
  ...
  // FIXME join结果错误 join key float type
  // TODO
  //!== Correct Answer - 1 ==   == Spark Answer - 1
  // struct<count(1):bigint>    struct<count(1):bigint>
  //![14432]                    [11516]
  test("join") {
    val df = spark.read.parquet(filename)
    df.createOrReplaceTempView("t1")
  ...
  // FIXME Falling back exec: FileSourceScanExec: Data type conversion not implemented TimestampNTZType
  // TODO
  //  Expected :0
  //  Actual   :1
  //
...
//  Actual   :1
  ...
  // FIXME min/max aggregate ![-Infinity,NaN] [-Infinity,Infinity]
  // TODO
  /**
   * ==Results==
  ...
  for (op <- Seq("+", "-", "*", "/", "div")) {
    test(s"decimal math: $op") {
      withSQLConf("spark.auron.decimal.arithOp.enabled" -> "true") {
        val df = spark.read.parquet(filename)
  ...
  // FIXME Support IntegralDivide function
  test(s"decimal math - div") {
    withSQLConf("spark.auron.decimal.arithOp.enabled" -> "true") {
      val df = spark.read.parquet(filename)
      val cols = df.schema.fields.filter(_.dataType.isInstanceOf[DecimalType]).map(_.name)
      df.createOrReplaceTempView("t1")

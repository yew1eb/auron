...
class AuronBitwiseExpressionSuite extends AuronQueryTest with BaseAuronSQLSuite {
  ...
  // FIXME unsupported expression: (class org.apache.spark.sql.catalyst.expressions.BitwiseXor)
  ...
  // FIXME unsupported expression: (class org.apache.spark.sql.catalyst.expressions.BitwiseGet)
  ...
  // FIXME unsupported expression: (class org.apache.spark.sql.catalyst.expressions.BitwiseCount)
  ...
  test("test function string_space") {
    withTable("t1") {
      sql("create table t1 using parquet as select 5 as num")
      val functions =
        s"""
           |select
           |  bit_count(num)

...
import org.apache.auron.testing.{DataGenOptions, FuzzDataGenerator}

class AuronStringExpressionSuite extends AuronQueryTest with BaseAuronSQLSuite {
  ...
  // FIXME support Various String scalar functions
  ...
  // FIXME unsupported expression: (class org.apache.spark.sql.catalyst.expressions.StringInstr)
  test("string repeat") {
    val table = "names"
    withTable(table) {
      sql(s"create table $table(id int, name varchar(20)) using parquet")

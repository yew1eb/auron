...
import org.apache.auron.testing.{DataGenOptions, DataGenerator, FuzzDataGenerator}
    ...
    Seq(false, true).foreach { dictionary =>
      withSQLConf("parquet.enable.dictionary" -> dictionary.toString) {
        ...
        withTable(table) {
          ...
          withView(view) {
            sql(
              s"""create temporary view $view as select c, v from values
                 | (null, null), (null, null),
                 | (null, 'S'), (null, 'S'),
                 | ('N', 'N '), ('N', 'N '),
                 | ('Ne', 'Sp'), ('Ne', 'Sp'),
            ...
            val df = sql(
              s"""select substring(c7, 2), substring(c8, 2),
    ...
    def testDoubleScalarExpr(expr: String): Unit = {
      ...
      for (withDictionary <- Seq(true, false)) {
        withParquetTable(testValuesRepeated.map(n => (n, n)), "tbl", withDictionary) {
          val (_, cometPlan) = checkSparkAnswerWithTolerance(sql(s"SELECT $expr(_1) FROM tbl"), 1e-6)
    ...
    test("named_struct with duplicate field names") {
      Seq(true, false).foreach { dictionaryEnabled =>
        withTempDir { dir =>
          ...
          withParquetTable(path.toString, "tbl") {
            checkSparkAnswerAndOperator(
              sql("SELECT named_struct('a', _1, 'a', _2) FROM tbl"))
            //classOf[ProjectExec])
            checkSparkAnswerAndOperator(
              sql("SELECT named_struct('a', _1, 'a', 2) FROM tbl"))
            // classOf[ProjectExec])
            checkSparkAnswerAndOperator(

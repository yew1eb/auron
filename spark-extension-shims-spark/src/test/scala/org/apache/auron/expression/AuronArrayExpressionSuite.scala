...
class AuronArrayExpressionSuite extends AuronQueryTest with BaseAuronSQLSuite {
  ...
  test("array_remove - test all types (native Parquet reader)") {
    withTempDir { dir =>
      withTempView("t1") {
        ...
        val fieldNames =
          table.schema.fields
            .filter(field => isTypeSupported(field.dataType))
            .map(_.name)
...
//    }
    withSQLConf(
      // CometConf.getExprAllowIncompatConfigKey(incompatKey) -> "true"
    ) {
      Seq(true, false).foreach { dictionaryEnabled =>
        withTempDir { dir =>
          withTempView("t1") {
...
// CometConf.getExprAllowIncompatConfigKey(classOf[ArrayDistinct]) -> "true"
  ...
  test("array_intersect") {
    withSQLConf(
      // CometConf.getExprAllowIncompatConfigKey(classOf[ArrayIntersect]) -> "true"
    ) {
      Seq(true, false).foreach { dictionaryEnabled =>
        withTempDir { dir =>
          withTempView("t1") {
  ...
  test("arrays_overlap") {
    withSQLConf(
      // ometConf.getExprAllowIncompatConfigKey(classOf[ArraysOverlap]) -> "true"
    ) {
      Seq(true, false).foreach { dictionaryEnabled =>
        withTempDir { dir =>
          withTempView("t1") {
  ...
  test("array_except - basic test (only integer values)") {
    withSQLConf(
      //CometConf.getExprAllowIncompatConfigKey(classOf[ArrayExcept]) -> "true"
    ) {
      Seq(true, false).foreach { dictionaryEnabled =>
        withTempDir { dir =>
          withTempView("t1") {
    ...
    withTempDir { dir =>
      ...
      withSQLConf(
        ...
        withTempView("t1", "t2") {
          ...
          // test with array of each column
          val fields =
            table.schema.fields.filter(field => isTypeSupported(field.dataType))
          for (field <- fields) {
            val fieldName = field.name
            val typeName = field.dataType.typeName
            sql(
...
//    val fallbackReason =
//      if (
//        //CometConf.COMET_NATIVE_SCAN_IMPL.key == CometConf.SCAN_NATIVE_COMET || sys.env

...
abstract class AuronTPCHSuite extends QueryTest with SharedSparkSession {
  ...
  protected val tpchResultsPath: String = s"$resourcePath/tpch-query-results"
  ...
  protected def createTPCHTables(): Unit = {
    tpchTables
      .foreach { tableName =>
        val tablePath = s"$tpchDataPath/$tableName"
        spark.read.parquet(tablePath).createOrReplaceTempView(tableName)
        val rowCount = spark.table(tableName).count()
        assert(rowCount > 0, s"TPC-H table $tableName is empty (path: $tablePath)")
      }
  }
  ...
  protected def compareResultStr(queryId: String, rows: Array[Row], goldenFile: File): Unit = {
    val actualContent = formatResultContent(rows)
    val expectedContent = FileUtils.readFileToString(goldenFile, StandardCharsets.UTF_8)

    if (expectedContent != actualContent) {
      fail(s"""=== TPC-H $queryId result mismatch ===
           |[Expected (golden file)]: ${goldenFile.getAbsolutePath}
           |[Actual]
           |$actualContent
           |[Expected]
           |$expectedContent
  ...
  protected def compareDoubleResult(
    ...
    assert(
      ...
      s"TPC-H $queryId row count mismatch: expected $expectedRowCount, got ${rows.length}")

    rows.zipWithIndex.foreach { case (actualRow, rowIdx) =>
      assert(expectedRowIter.hasNext, s"TPC-H $queryId missing expected row at index $rowIdx")
  ...
  protected def checkPhysicalPlan(df: DataFrame, queryId: String): Unit = {
    ...
    if (expectedPlan != actualPlan) {
      ...
      fail(s"""=== TPC-H $queryId physical plan mismatch ===
            |Expected file: ${goldenPlanFile.getAbsolutePath}
            |Actual file  : ${actualTempFile.getAbsolutePath}
            |--- Expected Plan ---
            |$expectedPlan
            |--- Actual Plan ---
            |$actualPlan
  ...
  def runTPCHQueryTest(queryId: String): Unit = {
